...
parser.add_argument("--device", type=str, default="0", help="Thiet bi chay YOLO: '0'=GPU, 'cpu'=CPU (dung khi GPU bi tranh voi Unity)")
...
MODEL_PATHS = [
    ...
    "/workspace/best3.pt",
    os.path.join(CURRENT_DIR, "Road_Seg_Model", "modelYolo", "weights", "best3.pt"),
    "Road_Seg_Model/modelYolo/weights/best3.pt"
]
if args.model:
...
def show_image_safe(win_name, img):
    ...
    if SHOW_GUI:
        try:
            # Thu nhỏ ảnh 50% để hiển thị GUI (X11) mượt hơn, tránh tụt FPS
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # Giam toc do khi model khong chac chan (error lon = bi den duong danh lua)
            if abs(blended_error) > 60:
                speed = min(speed, 20.0)

            try:
                AVControl(float(speed), float(smoothed_angle))
                time.sleep(0.025)  # Gioi han 40 lenh/giay, tranh flood socket Unity V2
            except Exception as e:
                if frame_count % 10 == 0:
